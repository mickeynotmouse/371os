use x86_64::structures::idt::{InterruptDescriptorTable, InterruptStackFrame, PageFaultErrorCode};
use spin::Mutex;
use pic8259::ChainedPics;
use pc_keyboard::{layouts, HandleControl, Keyboard, ScancodeSet1};
use core::sync::atomic::{AtomicUsize, AtomicBool, Ordering};
use crate::gdt;

pub const PIC_1_OFFSET: u8 = 32;
pub const PIC_2_OFFSET: u8 = PIC_1_OFFSET + 8;

#[repr(u8)]
#[derive(Debug, Clone, Copy)]
pub enum InterruptIndex {
    Timer = PIC_1_OFFSET,
    Keyboard = PIC_1_OFFSET + 1,
}

pub static GAME_STARTED: core::sync::atomic::AtomicBool =
    core::sync::atomic::AtomicBool::new(false);

pub static PICS: Mutex<ChainedPics> =
    Mutex::new(unsafe { ChainedPics::new(PIC_1_OFFSET, PIC_2_OFFSET) });

static mut IDT: InterruptDescriptorTable = InterruptDescriptorTable::new();

pub static mut KB_RAW: Keyboard<layouts::Us104Key, ScancodeSet1> = Keyboard::new(
    ScancodeSet1::new(),
    layouts::Us104Key,
    HandleControl::Ignore,
);

pub static TICKS: AtomicUsize = AtomicUsize::new(0);
pub static START_TICKS: AtomicUsize = AtomicUsize::new(0);
pub static CLOCK_RUNNING: AtomicBool = AtomicBool::new(false);
pub static DIGIT_COUNT: AtomicUsize = AtomicUsize::new(0);
pub static INPUT_DIGITS: Mutex<[u8; 6]> = Mutex::new([0u8; 6]);

pub fn init_idt() {
    unsafe {
        (*core::ptr::addr_of_mut!(IDT))
            .breakpoint
            .set_handler_fn(breakpoint_handler);
        (*core::ptr::addr_of_mut!(IDT))
            .double_fault
            .set_handler_fn(double_fault_handler)
            .set_stack_index(gdt::DOUBLE_FAULT_IST_INDEX);
        (*core::ptr::addr_of_mut!(IDT))
            .page_fault
            .set_handler_fn(page_fault_handler);
        (&mut (*core::ptr::addr_of_mut!(IDT)))
            [InterruptIndex::Timer as usize]
            .set_handler_fn(timer_handler);
        (&mut (*core::ptr::addr_of_mut!(IDT)))
            [InterruptIndex::Keyboard as usize]
            .set_handler_fn(keyboard_interrupt_handler);
        (*core::ptr::addr_of_mut!(IDT)).load();
    }
}

extern "x86-interrupt" fn breakpoint_handler(_stack_frame: InterruptStackFrame) {
    crate::serial_println!("EXCEPTION: BREAKPOINT");
}

extern "x86-interrupt" fn double_fault_handler(sf: InterruptStackFrame, _err: u64) -> ! {
    crate::serial_println!("DOUBLE FAULT: {:#?}", sf);
    crate::println!("DOUBLE FAULT");
    loop {}
}

extern "x86-interrupt" fn page_fault_handler(
    stack_frame: InterruptStackFrame,
    error_code: PageFaultErrorCode,
) {
    crate::println!("EXCEPTION: PAGE FAULT");
    crate::println!(
        "Accessed Address: {:?}",
        x86_64::registers::control::Cr2::read()
    );
    crate::println!("Error Code: {:?}", error_code);
    crate::println!("{:#?}", stack_frame);
    loop {}
}

extern "x86-interrupt" fn timer_handler(_stack_frame: InterruptStackFrame) {
    let ticks = TICKS.fetch_add(1, Ordering::Relaxed) + 1;
    // Update snake every 9 ticks (~18hz / 9 = ~2 updates per second)
    if ticks % 9 == 0 {
        if GAME_STARTED.load(Ordering::Relaxed) {
            crate::snake::update();
        }
    }
    unsafe {
        PICS.lock().notify_end_of_interrupt(InterruptIndex::Timer as u8);
    }
}

extern "x86-interrupt" fn keyboard_interrupt_handler(_stack_frame: InterruptStackFrame) {
    let scancode: u8 = unsafe { x86_64::instructions::port::Port::new(0x60).read() };

    if let Ok(Some(key_event)) = unsafe { (*core::ptr::addr_of_mut!(KB_RAW)).add_byte(scancode) } {
        if let Some(key) = unsafe { (*core::ptr::addr_of_mut!(KB_RAW)).process_keyevent(key_event) } {
            match key {
                pc_keyboard::DecodedKey::Unicode(c) => {
                    let current_dir = crate::snake::DIRECTION.load(Ordering::Relaxed);
                    match c {
                        'w' | 'W' => {
                            let already_going = current_dir == crate::snake::DIR_UP;
                            crate::snake::DIRECTION.store(crate::snake::DIR_UP, Ordering::Relaxed);
                            if GAME_STARTED.load(Ordering::Relaxed) && already_going {
                                crate::snake::update();
                            } else {
                                GAME_STARTED.store(true, Ordering::Relaxed);
                                // clear the start message on first keypress
                                clear_start_message();
                            }
                        }
                        'a' | 'A' => {
                            let already_going = current_dir == crate::snake::DIR_LEFT;
                            crate::snake::DIRECTION.store(crate::snake::DIR_LEFT, Ordering::Relaxed);
                            if GAME_STARTED.load(Ordering::Relaxed) && already_going {
                                crate::snake::update();
                            } else {
                                GAME_STARTED.store(true, Ordering::Relaxed);
                                clear_start_message();
                            }
                        }
                        's' | 'S' => {
                            let already_going = current_dir == crate::snake::DIR_DOWN;
                            crate::snake::DIRECTION.store(crate::snake::DIR_DOWN, Ordering::Relaxed);
                            if GAME_STARTED.load(Ordering::Relaxed) && already_going {
                                crate::snake::update();
                            } else {
                                GAME_STARTED.store(true, Ordering::Relaxed);
                                clear_start_message();
                            }
                        }
                        'd' | 'D' => {
                            let already_going = current_dir == crate::snake::DIR_RIGHT;
                            crate::snake::DIRECTION.store(crate::snake::DIR_RIGHT, Ordering::Relaxed);
                            if GAME_STARTED.load(Ordering::Relaxed) && already_going {
                                crate::snake::update();
                            } else {
                                GAME_STARTED.store(true, Ordering::Relaxed);
                                clear_start_message();
                            }
                        }
                        'q' | 'Q' => {
                            crate::qemu_quit(crate::QEMU_FAIL);
                        }
                        _ => {}
                    }
                }
                _ => {}
            }
        }
    }

    unsafe {
        PICS.lock().notify_end_of_interrupt(InterruptIndex::Keyboard as u8);
    }
}

fn clear_start_message() {
    for col in 0..80 {
        crate::snake::write_char(23, col, b' ', 0x0F);
    }
}
